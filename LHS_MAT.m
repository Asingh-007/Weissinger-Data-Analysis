...
for i = 1:panel_number
    ...
    for j = 1:panel_number
        ...
        y_horseshoe_1{i,j} = cross(u_inf, r_bound_1) / (norm(r_bound_1) * (norm(r_bound_2) - dot(u_inf, r_bound_1)));
        y_horseshoe_2{i,j} = cross(u_inf, r_bound_1) / (norm(r_bound_2) * (norm(r_bound_2) - dot(u_inf, r_bound_2)));
...
if dihedral_twist_correction % Corrects Vortex Coefficients for Twist and Dihedral Angle
    ...
    for i = 1:panel_number
        ...
        for j = 1:panel_number/2
            ...
            LHS_matrix(i,j) = -LHS_preliminary_matrix_x(i,j) * sin(-twist(i)) * cos(deg2rad(dihedral_angle))...
                - LHS_preliminary_matrix_y(i,j) * cos(-twist(i)) * sin(deg2rad(dihedral_angle))...
                + LHS_preliminary_matrix_z(i,j) * cos(-twist(i)) * cos(deg2rad(dihedral_angle));
